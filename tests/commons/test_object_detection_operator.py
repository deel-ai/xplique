...
from tests.utils import almost_equal, generate_data, generate_object_detection_model
...
def test_all_object_detector_operators():
    ...
    # create operators
    def normal_op(model, inputs, targets):
        return object_detection_operator(model, inputs, targets, **parameters_normal)

    def intersection_op(model, inputs, targets):
        return object_detection_operator(model, inputs, targets, **parameters_intersection)

    def probability_op(model, inputs, targets):
        return object_detection_operator(model, inputs, targets, **parameters_probability)

    def classification_op(model, inputs, targets):
        return object_detection_operator(model, inputs, targets, **parameters_classification)

    # compute explanations
    phis = Occlusion(model, operator=object_detection_operator, patch_size=4, patch_stride=2)(
        x, obj_ref
    )

    normal_phis = Occlusion(model, operator=normal_op, patch_size=4, patch_stride=2)(x, obj_ref)

    intersection_phis = Occlusion(model, operator=intersection_op, patch_size=4, patch_stride=2)(
        x, obj_ref
    )

    probability_phis = Occlusion(model, operator=probability_op, patch_size=4, patch_stride=2)(
        x, obj_ref
    )

    classification_phis = Occlusion(
        model, operator=classification_op, patch_size=4, patch_stride=2
    )(x, obj_ref)
