...
from xplique.utils_functions.object_detection.tf.box_manager import TfBoxManager, TfBoxCoordinatesTranslator
...
def test_normalize_boxes():
    ...
    expected_boxes = tf.constant([[0.25, 0.25, 0.5, 0.5], [0.15, 0.15, 0.3, 0.3]], dtype=tf.float32)
...
def test_box_cxcywh_to_xyxy():
    ...
    expected_boxes = tf.constant([[40, 40, 60, 60], [0.25, 0.25, 0.35, 0.35]], dtype=tf.float32)
...
def test_box_xywh_to_xyxy():
    ...
    xywh_boxes = tf.constant([[40, 40, 20, 20], [0.25, 0.25, 0.1, 0.1]], dtype=tf.float32)
...
def test_denormalize_boxes():
    ...
    normalized_boxes = tf.constant([[0.25, 0.25, 0.5, 0.5], [0.15, 0.15, 0.3, 0.3]], dtype=tf.float32)
    ...
    expected_boxes = tf.constant([[100, 100, 200, 200], [60, 60, 120, 120]], dtype=tf.float32)
