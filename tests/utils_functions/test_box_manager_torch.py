"""
Tests for PyTorch BoxManager and BoxCoordinatesTranslator.
"""
import torch
...
from xplique.utils_functions.object_detection.torch.box_manager import (
    TorchBoxManager, TorchBoxCoordinatesTranslator
)
...
def test_normalize_boxes():
    ...
    expected_boxes = torch.tensor(
        [[0.25, 0.25, 0.5, 0.5], [0.15, 0.15, 0.3, 0.3]], dtype=torch.float32
    )
...
def test_box_cxcywh_to_xyxy():
    ...
    expected_boxes = torch.tensor(
        [[40, 40, 60, 60], [0.25, 0.25, 0.35, 0.35]], dtype=torch.float32
    )
...
def test_box_xywh_to_xyxy():
    ...
    xywh_boxes = torch.tensor(
        [[40, 40, 20, 20], [0.25, 0.25, 0.1, 0.1]], dtype=torch.float32
    )
...
def test_denormalize_boxes():
    ...
    expected_boxes = torch.tensor(
        [[100, 100, 200, 200], [60, 60, 120, 120]], dtype=torch.float32
    )
