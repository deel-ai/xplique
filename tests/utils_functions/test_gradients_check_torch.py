...
class TorchvisionFormatModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(batch_size):
            ...
            results.append({"boxes": out, "scores": out * 2, "labels": out * 3})
        return results
...
class NestedDictModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.linear(pooled)
        return {
            "predictions": {"scores": out, "features": out * 2},
...
def test_model_with_input_preprocessing_breaking_gradients(input_tensor):
    ...
    class PreprocessingModel(torch.nn.Module):
        ...
        def forward(self, x):
            ...
            padded = torch.zeros(
                batch_size, channels, height + 10, width + 10, device=x.device, dtype=x.dtype
            )
    ...
    result = check_model_gradients(model, input_tensor)
    assert not result, (
        "Should detect broken gradients due to input preprocessing with torch.zeros()"
    )
