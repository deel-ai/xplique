    ...
    for input_shape in input_shapes:
        ...
        for perturb in perturbations:
            method = SobolAttributionMethod(
                model, grid_size=2, nb_design=8, perturbation_function=perturb
            )
            stis_maps = method.explain(x, y)
...
@pytest.mark.parametrize(
    "Estimator", [JansenEstimator, HommaEstimator, JanonEstimator, SaltelliEstimator]
)
def test_ishigami(Estimator):
    """Ensure every proposed estimator compute the correct Sobol total indice estimator
    on the ishigami function"""
...
def test_permutation_consistency(Estimator):
    ...
    masks_perm = tf.reshape(tf.gather(tf.reshape(masks[0, ..., 0], [-1]), perm), [grid, grid])[
        None, ..., None
    ]
