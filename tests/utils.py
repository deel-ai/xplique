...
def generate_model(input_shape=(32, 32, 3), output_shape=10):
    model = Sequential()
    model.add(Input(shape=input_shape))  # This does not add an input layer!
...
def generate_linear_model(features_coef, library: str = "keras"):
    if library == "sklearn":
...
def generate_object_detection_model(
    input_shape=(32, 32, 3), max_nb_boxes=10, nb_labels=5, with_nmf=False
):
    ...
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Flatten())
    model.add(Dense(int(np.prod(output_shape))))
    ...
    def valid_model(inputs):
        return make_plausible_boxes(model(inputs))
...
def download_file(identifier: str, destination: str):
    """
...
def get_gaussian_data(nb_classes=3, nb_samples_class=20, n_dims=1):
    ...
    X = tf.concat(
        [
            tf.random.normal(
                shape=(nb_samples_class, n_dims), mean=mu[i], stddev=sigma, dtype=tf.float32
            )
    ...
    y = tf.concat(
        [tf.ones(shape=(nb_samples_class), dtype=tf.int32) * i for i in range(nb_classes)], axis=0
    )
