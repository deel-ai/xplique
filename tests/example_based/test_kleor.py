...
from xplique.example_based.search_methods import KLEORGlobalSimSearch, KLEORSimMissSearch


def test_kleor_base_and_sim_miss():
    """
    Test suite for both the BaseKLEOR and KLEORSimMiss class. Indeed, the KLEORSimMiss
    class is a subclass of the BaseKLEOR class with a very basic implementation of the
    only abstract method (identity function).
    """
    # setup the tests
    cases = tf.constant(
    ...
    kleor = KLEORSimMissSearch(
        cases_dataset,
        cases_targets_dataset,
        k=1,
        search_returns=["examples", "indices", "distances", "include_inputs", "nuns"],
        batch_size=2,
    )
    ...
    # test the _filter_fn_nun method, this time the mask should be True when the targets
    # are different
    expected_mask = tf.constant(
        [
            [False, True, False, True, True],
    ...
    sf_indices, input_sf_distances, nun_sf_distances, batch_indices = kleor._initialize_search(
        inputs
    )
...
def test_kleor_global_sim():
    """
    Test suite for the KleorGlobalSim class. As only the kneighbors, format_output are
    impacted by the _additionnal_filtering method we test those 3 methods.
    """
    # setup the tests
    cases = tf.constant(
        [[1.0, 2.0], [2.0, 3.0], [3.0, 4.0], [4.0, 5.0], [5.0, 6.0]], dtype=tf.float32
    ...
    # (n, bs)
    fake_nun_sf_distances = tf.constant([[1.0, 2.0], [2.0, 3.0], [3.0, 4.0]])
    # (n, bs)
    fake_input_sf_distances = tf.constant([[2.0, 1.0], [3.0, 2.0], [2.0, 5.0]])
    # (n,1)
    fake_nuns_input_distances = tf.constant([[3.0], [1.0], [4.0]])
    ...
    expected_nun_sf_distances = tf.constant(
        [[1.0, 2.0], [np.inf, np.inf], [3.0, np.inf]], dtype=tf.float32
    )
    expected_input_sf_distances = tf.constant(
        [[2.0, 1.0], [np.inf, np.inf], [2.0, np.inf]], dtype=tf.float32
    )

    nun_sf_distances, input_sf_distances = kleor._additional_filtering(
        fake_nun_sf_distances, fake_input_sf_distances, fake_nuns_input_distances
    )
    ...
    assert tf.reduce_all(
        tf.abs(
            tf.where(inf_mask_nun_sf, 0.0, nun_sf_distances)
            - tf.where(inf_mask_expected_nun_sf, 0.0, expected_nun_sf_distances)
        )
        < 1e-5
    )
    ...
    assert tf.reduce_all(
        tf.abs(
            tf.where(inf_mask_input_sf, 0.0, input_sf_distances)
            - tf.where(inf_mask_expected_input_sf, 0.0, expected_input_sf_distances)
        )
        < 1e-5
    )
    ...
    expected_distances = tf.constant(
        [[kleor.fill_value], [np.sqrt(2 * 0.5**2)], [kleor.fill_value]], dtype=tf.float32
    )
    ...
    assert tf.reduce_all(
        tf.abs(
            tf.where(inf_mask_input, 0.0, input_sf_distances)
