...
def test_cole_attribution():
    ...
    def euclidean_dist(x, z):
        return tf.sqrt(tf.reduce_sum(tf.square(x - z), axis=-1))
...
def test_cole_hadamard():
    ...
    # Cole with Hadamard projection explain batch gradient is overwritten for test
    # purpose, do not copy!
    def weights_extraction(inputs, targets):
        return gradients_predictions(model, inputs, targets)
