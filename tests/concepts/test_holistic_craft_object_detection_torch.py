...
class MockTorchvisionLatentData(LatentData):
    ...
    def __init__(self, activations: np.ndarray, batch_size=1, device='cpu'):
        """
        Parameters
        ----------
        activations : np.ndarray
            Initial activations in (N, H, W, C) format.
        batch_size : int
            Batch size
...
class MockExtractorBuilder:
    ...
    def build(cls, model, device='cpu', batch_size=1):
        ...
        def input_to_latent_fn(inputs):
            ...
            return MockTorchvisionLatentData(activations=activations,
...
def test_latent_extractor(image_data, dataset_classes, latent_extractor_data):
    ...
    assert filtered_results.shape == torch.Size([3, 25])
...
def test_craft_reencode(image_data, dataset_classes, craft_data):
    ...
    assert filtered_result.shape == torch.Size([3, 25])

def test_craft_decoder_modes(image_data, dataset_classes, craft_data, device_param):
    """Test CRAFT encode/decode functionality."""
    image, input_tensor = image_data
    classes_names, nb_classes, label_to_color = dataset_classes
    craft = craft_data
...
def test_craft_importance_gradient_input(image_data, dataset_classes, craft_data):
    """Test CRAFT importance estimation by gradient input."""
    image, input_tensor = image_data
    classes_names, nb_classes, label_to_color = dataset_classes
    craft = craft_data
