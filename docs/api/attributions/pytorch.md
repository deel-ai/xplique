...
One thing to keep in mind is that **attribution methods expect a specific inputs format** as described in the [API Description](api_attributions.md#inputs-and-data-types). Especially, for images `inputs` should be $(N, H, W, C)$ following the TF's conventions where:
