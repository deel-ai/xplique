...
class GradCAM(WhiteBoxExplainer):
    ...
    def __init__(
        ...
        self.multi_output_model = tf.keras.Model(
            model.inputs, [self.conv_layer.output] + model.outputs
        )
    ...
    def explain(
        ...
        for x_batch, y_batch in tf.data.Dataset.from_tensor_slices((inputs, targets)).batch(
            ...
            batch_feature_maps, batch_gradients = self._gradient(x_batch, y_batch)
    ...
    # @tf.function
    def _gradient(self, inputs: tf.Tensor, targets: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        """
        ...
        with tf.GradientTape() as tape:
            # watched_layer = watch_layer(self.conv_layer, tape)
            tape.watch(inputs)
            predictions = self.multi_output_model(inputs)
            score = tf.reduce_sum(tf.multiply(predictions, targets), axis=-1)

        feature_maps_gradients = tape.gradient(score, inputs)
        feature_maps = watched_layer.result
        unwatch_layer(self.conv_layer)
