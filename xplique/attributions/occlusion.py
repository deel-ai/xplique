...
class Occlusion(BlackBoxExplainer):
    ...
    def _get_masks(
        ...
        if is_tabular:
            x_anchors = [
                x * patch_stride
                for x in range(0, ceil((input_shape[0] - patch_size + 1) / patch_stride))
            ]
            ...
            x_anchors = [
                x * patch_stride[0]
                for x in range(0, ceil((input_shape[0] - patch_size[0] + 1) / patch_stride[0]))
            ]
            y_anchors = [
                y * patch_stride[1]
                for y in range(0, ceil((input_shape[1] - patch_size[1] + 1) / patch_stride[1]))
            ]
            ...
            for x_anchor in x_anchors:
                for y_anchor in y_anchors:
                    ...
                    mask[
                        x_anchor : x_anchor + patch_size[0], y_anchor : y_anchor + patch_size[1]
                    ] = 1
                    masks.append(mask)
