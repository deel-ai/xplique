...
class LatentDataRetinanet(LatentData):
    """
    Stores latent representations (features and metadata) from RetinaNet's backbone.

    This class encapsulates the multi-scale feature maps from a RetinaNet model,
    ...
    def __init__(self,
        ...
        """
        Initialize RetinaNet latent data with images, sizes, and features.
    ...
    def to(self, device: torch.device) -> 'LatentData':
        ...
        return LatentDataRetinanet(images, self.original_image_sizes, features,
...
class RetinanetExtractorBuilder(LatentExtractorBuilder):
    ...
    configured for RetinaNet object detection models. It defines the forward pass
    split into backbone feature extraction (g) and FPN + detection head processing (h).
    """

    @classmethod
    def build(cls, model, device: str = 'cuda', nb_classes: int = 91,
              extraction_layer: int = -1, extract_location: str = 'resnet') -> 'TorchLatentExtractor':
        """
        ...
        them through the Feature Pyramid Network (FPN) and RetinaNet detection head.

        Parameters
        ----------
        model
            PyTorch RetinaNet model instance with backbone, head, anchor_generator,
        ...
        -------
        latent_extractor
            Configured TorchLatentExtractor instance for the RetinaNet model.
        """

        def g(self, images):
            # See https://docs.pytorch.org/vision/main/_modules/torchvision/models/detection/retinanet.html#retinanet_resnet50_fpn
            targets = None
            ...
            return LatentDataRetinanet(
                images,
                original_image_sizes,
                features,
                extraction_layer=extraction_layer)

        def h(self, latent_data: LatentDataRetinanet):
        ...
        print(f"Building RetinaNet extractor: extracting from '{extract_location}' at layer index {extraction_layer}")
        ...
        latent_extractor = TorchLatentExtractor(
            ...
            model.g,
            model.h,
            latent_data_class=LatentDataRetinanet,
