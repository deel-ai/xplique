...
class LatentDataLayered(LatentData):
    ...
    def get_activations(self, as_numpy: bool = True, keep_gradients: bool = False) -> Union[np.ndarray, tf.Tensor]:
        """
        ...
        if as_numpy:
            activations = activations.numpy() if hasattr(activations, 'numpy') else np.array(activations)
